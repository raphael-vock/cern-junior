...
\noindent Pour représenter l'\textit{objet} correspondant au champ vectoriel nous voulons une méthode générique pour pouvoir le dessiner. On va donc encapsuler un attribut de type \T{VectorFunction} dans une sous-classe de la class abstraite \T{Drawable}. Par ailleurs le C++ permet de surcharger l'opérateur d'appel ce qui permet de maintenir l'interface ``fonctionnelle'' de l'objet.
\begin{align*}
